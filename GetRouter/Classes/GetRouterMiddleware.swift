...
extension GetRouterMiddleware {
    /// 优先级
    var priority: Int { 1 }
    
    /// 处理返回结果
    /// - Returns: true: 允许通过 false: 禁止
    func handler(routeName: GetRouterName, params: GetDict?) -> Bool { true }
    
    @available(iOS 13.0.0, *)
    /// 异步中间件, 注意如果想要在此出重定向路由,注意要回到mainThread
    /// - Parameter routeName: 路由名
    /// - Returns: true: 允许通过 false: 禁止
    func handlerAsync(routeName: GetRouterName, params: GetDict?) async throws -> Bool { true }
}
